...
import { Phone, Zap, Wallet, ArrowRight } from "lucide-react"
					...
					<div className="grid md:grid-cols-3 gap-8">
						{howItWorksSteps.map((step, index) => {
							...
							return (
								<Card
									key={step.step}
									className="minimal-card minimal-card-hover border-0"
								>
				...
				<div className="container mx-auto max-w-6xl">
					<div className="text-center mb-16">
						...
						<p className="text-lg text-gray-600">
							Top up airtime for all major Nigerian telecom providers
						</p>
					</div>
						...
						{supportedNetworks.map((network) => (
							...
							>
								<CardContent className="p-6 text-center">
									...
									<h3 className={`font-semibold ${network.color}`}>
										{network.name}
									</h3>
								</CardContent>
							</Card>
					...
					<div className="grid grid-cols-2 md:grid-cols-4 gap-6">
						{cryptoTokens.map((token) => (
							...
							>
								<CardContent className="p-6 text-center">
									<div
										className={`text-4xl mb-3 font-bold ${token.color}`}
									>
										{token.symbol}
									</div>
